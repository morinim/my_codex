...
(dolist (autoload-entry
         '((my-codex-send-region . "my-codex-prompts")
           ...
           (my-codex-explain-problem . "my-codex-problems")
           (my-codex-explain-compilation-error-at-point . "my-codex-problems")
  ...
  (defun my-codex--command-entry-with-defaults (entry)
    ...
    (append
     ...
     (pcase (plist-get entry :prefix)
       ...
       ('my-codex-ask-preset-transient '(:group "Send" :path "A"))
  ...
  (defconst my-codex-command-catalogue
    (mapcar
     ...
     '((:command my-codex-read-only :key "o" :label "Read-only" :group "Session" :menu "Show/start read-only" :help "Show the configured agent in read-only mode")
      ...
      (:command my-codex-explain-problem :key "e" :label "Explain problem" :group "Context" :menu "Explain problem at point" :contexts (code terminal unknown) :help "Explain the selected error, diagnostic, or compilation problem at point")
