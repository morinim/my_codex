...
(ert-deftest my-codex-setup-detects-available-agent-executables ()
  (let ((my-codex-agent-profiles
         '((first :commands ((read-only . "first --safe")))
           (second :commands ((read-only . "env MODE=safe second"))))))
    (cl-letf (((symbol-function 'executable-find)
               (lambda (program)
                 (and (equal program "second") "/bin/second"))))
      (should (equal (my-codex--setup-available-agents) '(second))))))

(ert-deftest my-codex-setup-orders-terminal-backends-by-platform ()
  (cl-letf (((symbol-function 'my-codex-default-terminal-backend)
             (lambda () 'eat))
            ((symbol-function 'my-codex--terminal-backend-loadable-p)
             (lambda (backend) (memq backend '(eat vterm)))))
    (should (equal (my-codex--setup-available-backends) '(eat vterm)))))

(ert-deftest my-codex-setup-saves-selections-enables-mode-and-runs-doctor ()
  (let (saved enabled doctor-ran)
    (cl-letf (((symbol-function 'my-codex--setup-available-agents)
               (lambda () '(antigravity)))
              ((symbol-function 'my-codex--setup-available-backends)
               (lambda () '(eat)))
              ((symbol-function 'customize-save-variable)
               (lambda (variable value)
                 (push (list variable value) saved)))
              ((symbol-function 'my-codex-global-mode)
               (lambda (value) (setq enabled value)))
              ((symbol-function 'my-codex-doctor)
               (lambda () (setq doctor-ran t))))
      (my-codex-setup))
    (should
     (equal saved
            '((my-codex-terminal-backend eat)
              (my-codex-agent antigravity))))
    (should (= enabled 1))
    (should doctor-ran)))

(ert-deftest my-codex-setup-restarts-an-active-mode ()
  (let ((my-codex-global-mode t)
        mode-calls)
    (cl-letf (((symbol-function 'my-codex--setup-available-agents)
               (lambda () '(codex)))
              ((symbol-function 'my-codex--setup-available-backends)
              ...
              ((symbol-function 'customize-save-variable)
               #'ignore)
              ((symbol-function 'my-codex-global-mode)
               (lambda (value) (push value mode-calls)))
              ((symbol-function 'my-codex-doctor)
               #'ignore))
      (my-codex-setup))
    (should (equal (nreverse mode-calls) '(-1 1)))))
...
(ert-deftest my-codex-new-session-defaults-to-read-only ()
  (let (started)
    (cl-letf (((symbol-function 'my-codex-two-column-layout-with-command)
               (lambda (&rest args)
                 (setq started args))))
      (my-codex-new-session "plan" 'antigravity))
    (should (equal (nth 0 started)
                   (my-codex--agent-command 'antigravity 'read-only)))
    (should-not (nth 1 started))
    (should (equal (nth 2 started) "plan"))
    (should (eq (nth 3 started) 'antigravity))
    (should (eq (nth 4 started) 'read-only))))
