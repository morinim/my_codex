  ...
  (let ((my-codex-after-send-action 'focus)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'get-buffer-window)
                   (lambda (target all-frames)
                     (should (eq target buffer))
                     (should all-frames)
                     'agent-window))
                  ((symbol-function 'select-window)
                   (lambda (window &rest _args) (setq selected window)))
                  ((symbol-function 'pop-to-buffer)
                   (lambda (&rest _args) (ert-fail "Unexpected pop"))))
          (my-codex--display-after-send buffer)
          (should (eq selected 'agent-window)))
      (kill-buffer buffer))))

(ert-deftest my-codex-display-after-send-pops-to-hidden-agent ()
  (let ((my-codex-after-send-action 'focus)
        (buffer (get-buffer-create "*my-codex-hidden-agent*"))
        popped)
    (unwind-protect
        (cl-letf (((symbol-function 'get-buffer-window) #'ignore)
                  ((symbol-function 'pop-to-buffer)
                   (lambda (target &rest _args) (setq popped target))))
          (my-codex--display-after-send buffer)
          (should (eq popped buffer)))
      (kill-buffer buffer))))

(ert-deftest my-codex-display-after-send-displays-without-selecting ()
  (let ((my-codex-after-send-action 'display)
        (my-codex-display-buffer-action 'test-action)
        (buffer (get-buffer-create "*my-codex-displayed-agent*"))
        displayed)
    (unwind-protect
        (cl-letf (((symbol-function 'display-buffer)
                   (lambda (target action)
                     (setq displayed (list target action))))
                  ((symbol-function 'select-window)
                   (lambda (&rest _args) (ert-fail "Unexpected selection")))
                  ((symbol-function 'pop-to-buffer)
                   (lambda (&rest _args) (ert-fail "Unexpected pop"))))
          (my-codex--display-after-send buffer)
          (should (equal displayed (list buffer 'test-action))))
      (kill-buffer buffer))))

(ert-deftest my-codex-display-after-send-can-stay-in-current-window ()
  (let ((my-codex-after-send-action 'stay)
        (buffer (get-buffer-create "*my-codex-undisplayed-agent*")))
    (unwind-protect
        (cl-letf (((symbol-function 'get-buffer-window)
                   (lambda (&rest _args) (ert-fail "Unexpected lookup")))
                  ...
                  ((symbol-function 'pop-to-buffer)
                   (lambda (&rest _args) (ert-fail "Unexpected pop"))))
          (should-not (my-codex--display-after-send buffer)))
      (kill-buffer buffer))))
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function
                    ...
                    ((symbol-function 'pop-to-buffer)
                     (lambda (&rest _args)
                       (ert-fail "Failed send displayed the agent")))
                    ((symbol-function 'my-codex-backend-send)
